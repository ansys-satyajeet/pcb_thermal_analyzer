...
def get_solution_name():
    solution_name = st.session_state.ipk.get_setups()[0] + ':' + st.session_state.ipk.post.post_solution_type
...
def get_monitor_point_temperatures(solution_name):
    ...
    st.session_state.ipk.post.oreportsetup.CreateReport(mon_point_table,"Monitor", "Data Table", solution_name, [], a, b)
    ...
    st.session_state.ipk.post.oreportsetup.ExportToFile(mon_point_table,os.path.join(os.getcwd(), mon_point_temp_file),False)
...
def get_network_junction_temperatures(solution_name):
    list_bcs = st.session_state.ipk.odesign.GetChildObject('Thermal').GetChildNames()
    thermal_bc_types = {}
    for i in list_bcs:
        l = []
...
def get_object_max_temperatures(solution_name):
    ...
    comp3d_instance_name = st.session_state.ipk.odesign.GetChildObject('3D Modeler').Get3DComponentInstanceNames(comp3d_name)[0]
    comp3d_part_names = list(st.session_state.ipk.odesign.GetChildObject('3D Modeler').Get3DComponentPartNames(comp3d_instance_name))
    ...
    st.session_state.ipk.post.oreportsetup.CreateReport(obj_max_temp,"Fields", "Data Table", solution_name, [], a, b)
    ...
    st.session_state.ipk.post.oreportsetup.ExportToFile(obj_max_temp,os.path.join(os.getcwd(), obj_max_temp_file),False)
